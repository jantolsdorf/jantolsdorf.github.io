import { D as DataType, s as scopedHtml, i as ifDefined, a as styleMap, b as setTags, c as setSuffix, h as classMap, u as repeat, A as unsafeHTML, r as registerThemePropertiesLoader, d as defaultThemeBase, e as defaultTheme, I as Integer, U as UI5Element, l as litRender, k as getFeature, o as renderFinished, f as fetchI18nBundle } from './Icons-85d4abff.js';
import { R as ResizeHandler, e as Popover } from './Popover-0d784de7.js';
import { g as getI18nBundle, p as isPhone, j as isUp, f as isDown, c as isSpace, d as isEnter, o as isEscape, w as isBackSpace, i as isIE, E as isSafari, F as VALUE_STATE_SUCCESS, H as VALUE_STATE_INFORMATION, V as VALUE_STATE_ERROR, l as VALUE_STATE_WARNING, I as INPUT_SUGGESTIONS_TITLE, a as getEffectiveAriaLabelText, J as INPUT_SUGGESTIONS, K as INPUT_SUGGESTIONS_MORE_HITS, N as INPUT_SUGGESTIONS_ONE_HIT, O as INPUT_SUGGESTIONS_NO_HIT } from './i18n-defaults-dca87248.js';
...
const block0$1 = (context) => { return scopedHtml`<div class="ui5-input-root" @focusin="${context._onfocusin}" @focusout="${context._onfocusout}"><div class="ui5-input-content"><input id="${ifDefined(context._id)}-inner" class="ui5-input-inner" style="${styleMap(context.styles.innerInput)}" type="${ifDefined(context.inputType)}" inner-input ?inner-input-with-icon="${context.icon.length}" ?disabled="${context.disabled}" ?readonly="${context._readonly}" .value="${ifDefined(context.value)}" placeholder="${ifDefined(context._placeholder)}" maxlength="${ifDefined(context.maxlength)}" role="${ifDefined(context.accInfo.input.role)}" aria-owns="${ifDefined(context.accInfo.input.ariaOwns)}" ?aria-invalid="${context.accInfo.input.ariaInvalid}" aria-haspopup="${ifDefined(context.accInfo.input.ariaHasPopup)}" aria-describedby="${ifDefined(context.accInfo.input.ariaDescribedBy)}" aria-roledescription="${ifDefined(context.accInfo.input.ariaRoledescription)}" aria-autocomplete="${ifDefined(context.accInfo.input.ariaAutoComplete)}" aria-expanded="${ifDefined(context.accInfo.input.ariaExpanded)}" aria-label="${ifDefined(context.accInfo.input.ariaLabel)}" aria-required="${ifDefined(context.required)}" @input="${context._handleInput}" @change="${context._handleChange}" @keydown="${context._onkeydown}" @keyup="${context._onkeyup}" @click=${context._click} @focusin=${context.innerFocusIn} data-sap-no-tab-ref data-sap-focus-ref step="${ifDefined(context.nativeInputAttributes.step)}" min="${ifDefined(context.nativeInputAttributes.min)}" max="${ifDefined(context.nativeInputAttributes.max)}" />${ context.icon.length ? block1$1(context) : undefined }${ context.showSuggestions ? block2$1(context) : undefined }${ context.accInfo.input.ariaDescription ? block3$1(context) : undefined }${ context.hasValueState ? block4$1(context) : undefined }</div><slot name="formSupport"></slot></div>`; };
const block1$1 = (context) => { return scopedHtml`<div class="ui5-input-icon-root"><slot name="icon"></slot></div>`; };
const block2$1 = (context) => { return scopedHtml`<span id="${ifDefined(context._id)}-suggestionsText" class="ui5-hidden-text">${ifDefined(context.suggestionsText)}</span><span id="${ifDefined(context._id)}-selectionText" class="ui5-hidden-text" aria-live="polite" role="status"></span><span id="${ifDefined(context._id)}-suggestionsCount" class="ui5-hidden-text" aria-live="polite">${ifDefined(context.availableSuggestionsCount)}</span>`; };
const block3$1 = (context) => { return scopedHtml`<span id="${ifDefined(context._id)}-descr" class="ui5-hidden-text">${ifDefined(context.accInfo.input.ariaDescription)}</span>`; };
const block4$1 = (context) => { return scopedHtml`<span id="${ifDefined(context._id)}-valueStateDesc" class="ui5-hidden-text">${ifDefined(context.ariaValueStateHiddenText)}</span>`; };
...
const block0 = (context) => { return scopedHtml`${ context.showSuggestions ? block1(context) : undefined }${ context.hasValueStateMessage ? block17(context) : undefined } `; };
const block1 = (context) => { return scopedHtml`<ui5-responsive-popover no-arrow _disable-initial-focus placement-type="Bottom" horizontal-align="Left" style="${styleMap(context.styles.suggestionsPopover)}" @ui5-after-open="${ifDefined(context._afterOpenPopover)}" @ui5-after-close="${ifDefined(context._afterClosePopover)}" @ui5-scroll="${ifDefined(context._scroll)}">${ context._isPhone ? block2(context) : undefined }${ !context._isPhone ? block7(context) : undefined }<ui5-list separators="${ifDefined(context.suggestionSeparators)}">${ repeat(context.suggestionsTexts, (item, index) => item._id || index, (item, index) => block12(item, index, context)) }</ui5-list>${ context._isPhone ? block16(context) : undefined }</ui5-responsive-popover>`; };
const block2 = (context) => { return scopedHtml`<div slot="header" class="ui5-responsive-popover-header"><div class="row"><span>${ifDefined(context._headerTitleText)}</span><ui5-button class="ui5-responsive-popover-close-btn" icon="decline" design="Transparent" @click="${context._closeRespPopover}"></ui5-button></div><div class="row"><div class="input-root-phone"><input class="ui5-input-inner-phone" type="${ifDefined(context.inputType)}" .value="${ifDefined(context.value)}" inner-input placeholder="${ifDefined(context.placeholder)}" @input="${context._handleInput}" @change="${context._handleChange}" /></div></div>${ context.hasValueStateMessage ? block3(context) : undefined }</div>`; };
const block3 = (context) => { return scopedHtml`<div class="row ${classMap(context.classes.popoverValueState)}" style="${styleMap(context.styles.suggestionPopoverHeader)}">${ context.shouldDisplayDefaultValueStateMessage ? block4(context) : block5(context) }</div>`; };
const block4 = (context) => { return scopedHtml`${ifDefined(context.valueStateText)}`; };
const block5 = (context) => { return scopedHtml`${ repeat(context.valueStateMessageText, (item, index) => item._id || index, (item, index) => block6(item, index, context)) }`; };
const block6 = (item, index, context) => { return scopedHtml`${ifDefined(item)}`; };
const block7 = (context) => { return scopedHtml`${ context.hasValueStateMessage ? block8(context) : undefined }`; };
const block8 = (context) => { return scopedHtml`<div slot="header" class="ui5-responsive-popover-header ${classMap(context.classes.popoverValueState)}" style=${styleMap(context.styles.suggestionPopoverHeader)}>${ context.shouldDisplayDefaultValueStateMessage ? block9(context) : block10(context) }</div>`; };
const block9 = (context) => { return scopedHtml`${ifDefined(context.valueStateText)}`; };
const block10 = (context) => { return scopedHtml`${ repeat(context.valueStateMessageText, (item, index) => item._id || index, (item, index) => block11(item, index, context)) }`; };
const block11 = (item, index, context) => { return scopedHtml`${ifDefined(item)}`; };
const block12 = (item, index, context) => { return scopedHtml`${ item.group ? block13(item, index, context) : block14(item, index, context) }`; };
...
const block19 = (context) => { return scopedHtml`${ repeat(context.valueStateMessageText, (item, index) => item._id || index, (item, index) => block20(item, index, context)) }`; };
...
export { Input as I, ResponsivePopoverCommonCss as R, ValueStateMessageCss as V, InputType as a };
